...
const props = withDefaults(
  defineProps<{ tooltip?: string; position?: 'top' | 'bottom' | 'left' | 'right'; contentClass?: string }>(),
  {
    tooltip: undefined,
    position: 'top',
    contentClass: '',
...
const { tooltip, position, contentClass } = toRefs(props);
...
<template>
  <div relative inline-block>
    ...
    <div
      ...
      class="absolute z-10 whitespace-nowrap rounded bg-black px-12px py-6px text-sm text-white shadow-lg transition transition transition-duration-0.2s" :class="[
        contentClass,
        {
