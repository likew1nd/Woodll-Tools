...
const { copy } = useCopy({ source: hashed, text: t('tools.bcrypt.hashCopied') });
...
<template>
  <c-card :title="$t('tools.bcrypt.hashTitle')">
    ...
    <n-form-item :label="$t('tools.bcrypt.saltCountLabel')" label-placement="left" label-width="120">
      <n-input-number v-model:value="saltCount" :placeholder="$t('tools.bcrypt.saltCountPlaceholder')" :max="100" :min="0" w-full />
    </n-form-item>
  ...
  <c-card :title="$t('tools.bcrypt.compareTitle')">
    <n-form label-width="120">
      ...
      <n-form-item :label="$t('tools.bcrypt.compareMatchLabel')" label-placement="left" :show-feedback="false">
        <div class="compare-result" :class="{ positive: compareMatch }">
          {{ compareMatch ? $t('tools.bcrypt.compareMatchYes') : $t('tools.bcrypt.compareMatchNo') }}
        </div>
      </n-form-item>
    </n-form>
  </c-card>
