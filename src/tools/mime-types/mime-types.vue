...
<template>
  <c-card>
    ...
    <div style="opacity: 0.8">
      {{ $t('tools.mime-types.mimeToExtensionSubtitle') }}
    </div>
    <c-select
      ...
      :placeholder="$t('tools.mime-types.mimeSelectPlaceholder')"
    />

    <div v-if="extensionsFound.length > 0">
      {{ $t('tools.mime-types.extensionsOf') }} <n-tag round :bordered="false">
        {{ selectedMimeType }}
  ...
  <c-card>
    ...
    <div style="opacity: 0.8">
      {{ $t('tools.mime-types.extensionToMimeSubtitle') }}
    </div>
    <c-select
    ...
    <div v-if="selectedExtension">
      {{ $t('tools.mime-types.mimeForExtensionPrefix') }} <n-tag round :bordered="false">
        {{ selectedExtension }}
      </n-tag> {{ $t('tools.mime-types.extensionSuffix') }}
      <div style="margin-top: 10px">
